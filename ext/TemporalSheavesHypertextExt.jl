...
"""
    to_graphviz_htl(graphs; node_labels=true, edge_labels=true,
                    style="display: flex; flex-wrap: wrap;",
                    graph_attrs=Dict(:splines=>"false", :rankdir=>"LR", :dpi=>"70"),
                    kwargs...)

Render a collection of Graphviz graphs as a single HypertextLiteral HTML
object, suitable for display in Documenter.jl `@example` blocks.

Each graph is converted to an SVG string and embedded as a raw HTML div,
laid out according to the `style` keyword argument.

# Example
```julia
subobs = subobject_graph_embedding(constructor)
to_graphviz_htl(subobs)
```
"""
